...
# Image cropping methods - Refer the Inception_v1 paper; central crop
import numpy as np
...
image_path = "train/000bec180eb18c7604dcecc8fe0dba07.jpg"

 
def image_resize_keep_aspect(image_path_, new_shorter_side = 300):
    """
    Input: image from glob
    Returns resized image preserving the aspect ratio. Then may be perform cropping
    ...
    #using width = tf.shape(image)[1] gave Expected float64, got list containing Tensors of type '_Message' instead. or list with multple #tensors 
    ...
    def compute_longer_side(initial_larger_side, initial_shorter_side, new_shorter_side):
        new_larger_side = int((new_shorter_side / initial_shorter_side) * initial_larger_side)
        new_larger_side = tf.constant(new_larger_side)
    ...
    dimensions = tf.cond(height_less_equal_width, 
                    lambda: [new_shorter_side, int(compute_longer_side(width,height,new_shorter_side))],
                    lambda: [int(compute_longer_side(height, width, new_shorter_side)), new_shorter_side])
    X = tf.placeholder(tf.float64, shape = [None, None, 3])
    tf_resized_image = tf.image.resize_images(X, dimensions, tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    sess = tf.Session()
...
image_resize_keep_aspect(image_path)
